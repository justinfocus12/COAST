    ...
    jldopen(joinpath(datadir,"xs_dscs.jld2"), "r") do f
        for i_anc = 1:min(2,N_anc)
            ...
            for i_ast = 1:N_ast
                ...
                ax3 = Axis(lout[i_ast,3]; ylabel="AST=$(asts[i_ast])", ylabelrotation=0, yticklabelsvisible=false, xlabel="δ𝑥(𝑡*−𝐴)×$(powerofhalfstring(perturbation_neglog))", title="𝑅*(δ𝑥)", theme_ax..., xticks=([-1,-1/2,0,1/2,1]./(2^perturbation_neglog), ["−1","−½","0","+½","+1"]))
                # Plot the ancestor
